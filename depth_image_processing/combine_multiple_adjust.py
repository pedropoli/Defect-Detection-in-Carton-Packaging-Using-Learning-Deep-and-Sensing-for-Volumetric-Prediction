            ...
            if target_id == source_id + 1:  # odometry case
                ...
                pose_graph.edges.append(
                    open3d.pipelines.registration.PoseGraphEdge(source_id,
                                                             ...
                                                             uncertain=True))
    return pose_graph
def remove_points_below_plane(point_cloud):
    threshold_plane = np.array([1, 2, 0, -2])
    ...
    signed_distances = np.dot(np.hstack((points_np, np.ones((len(points_np), 1)))), threshold_plane)

    # Keep only the points above the threshold plane
    filtered_points = points_np[signed_distances >= 0]

    # Create a new point cloud with the filtered points
    filtered_point_cloud = open3d.geometry.PointCloud()
    filtered_point_cloud.points = open3d.utility.Vector3dVector(filtered_points)
        ...
        if process_snapshot == -1:
            ...
            if cmd == 'p':
                ...
                for cloud in all_snapshots:
                    cropped_pc = remove_points_below_plane(cloud)
                    cropped_pc = demo_crop_geometry(cropped_pc)
                    list_cropped_pc.append(cropped_pc)
                    # open3d.visualization.draw_geometries([cropped_pc, plane_points])
